...
public class Caminos {
	...
	public List<Integer> caminoAHojaMasLejana () {
		...
		if (arbol != null) {
			caminoAHojaMasLejana(arbol, caminoMax, caminoAct);
		}
		return caminoMax;
	}
	
	private void caminoAHojaMasLejana (GeneralTree<Integer> arbol, List<Integer> caminoMax, List<Integer> caminoAct) {
		caminoAct.add(arbol.getData());
		if (!arbol.isLeaf()) {
			List <GeneralTree<Integer>> children = arbol.getChildren();
			for (GeneralTree<Integer> child: children) {
				caminoAHojaMasLejana (child, caminoAct, caminoMax);
			}
		}
		else {
